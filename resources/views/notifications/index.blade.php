                ...
                <div class="flex-grow-1">
                    ...
                    <div class="d-flex align-items-center mb-1">
                        ...
                        @switch($notification->type)
                            @case('activity_reminder') <i class="fas fa-clock text-warning me-2"></i> @break
                            @case('new_activity') <i class="fas fa-calendar-plus text-success me-2"></i> @break
                            @case('invitation') <i class="fas fa-handshake text-purple me-2"></i> @break
                            @default <i class="fas fa-bell text-primary me-2"></i>
                        @endswitch
                        ...
                        <div class="small mb-2">
                            ...
                            @if($notification->activity->start_date)
                                <span class="badge bg-success ms-1 mb-1">
                                    <i class="fas fa-clock me-1"></i>{{ $notification->activity->start_date->translatedFormat('d M Y H:i') }}
                                </span>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="small text-muted">
                        <i class="fas fa-calendar me-1"></i> {{ $notification->created_at->translatedFormat('d M Y H:i') }}
                        <span class="mx-2">•</span>
                        <i class="fas fa-broadcast-tower me-1"></i> {{ $notification->channel }}
                        @if($notification->data['minutes_before'] ?? false)
                            <span class="badge bg-warning text-dark ms-2">
                                <i class="fas fa-hourglass-half me-1"></i>{{ $notification->data['minutes_before'] }} menit sebelum
                            </span>
                        @endif
                    </div>
                </div>
